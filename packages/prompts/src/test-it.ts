...
export async function simulateBot(args: {
  ...
  const resp = await getClaude().messages.create({
    ...
    system: [
      { type: "text", text: args.botSystemPrompt, cache_control: { type: "ephemeral" } },
      {
        type: "text",
        text:
          "SIMULATION ONLY. This is a Test It preview, not a live customer. After your response, list referenced entry IDs on a final line: REFS: [id1,id2,...]",
      },
  ...
  const refs = refsMatch && refsMatch[1]
    ? refsMatch[1].split(",").map((s) => s.trim().replace(/["']/g, "")).filter(Boolean)
    : [];
